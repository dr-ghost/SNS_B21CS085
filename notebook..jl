...
# ╔═╡ f07ad2c7-c64b-4cf7-b82c-5cd37ccd5ded
using Plots, FFTW, DSP

# ╔═╡ 4f891a2c-3c89-4423-baa5-1b9dedf0c519
md"# Coding Assignment"

# ╔═╡ d06e566a-bad0-4be9-80dd-14c078265120
plotly()

# ╔═╡ e2bb2394-6749-4d0f-bc66-c6264069c67a
md"""## Question 1"""

# ╔═╡ 6b713ee5-c59a-42a3-9c8d-29913453d98f
begin
	function Xt(t, W)
	        return sin(W * t) / pi * t
	end
	md" function to generate our sinc signal"
end

# ╔═╡ 0648ecbe-a937-4e46-9440-3447ffd5f247
begin
	function Ut(t)
        if t >0.0
            return 1.0
	...
	md" function to generate our step signal"
end
...
# ╔═╡ 6c12ac8a-ae73-4160-954f-34fc95edab7c
begin
	w = -10.0:fs:10.0
    t = -10.0:fs:10.0
	
	md" time and frequency vars"
end

# ╔═╡ fd3ce808-807e-4ff9-9b99-c07a5264fd60
begin
	xt = Xt.(t,3)
	md" Generating our input signal"
end

# ╔═╡ fb25075e-3891-434f-b406-317f793e9141
begin
	p0 = plot()
	plot!(p0, t, xt, label = "x(t)", title = "x(t)")
end

# ╔═╡ 573aa3c7-86c0-4d69-bb95-d387a852f1d2
begin
	ut = Ut.(t)
	md" Generating Step function"
end

# ╔═╡ c9bf6b76-1e52-453a-bcc9-f308d755bac4
function Q1(xt)
        ht = exp.(-a * t) .* ut
...
# ╔═╡ 66ef8993-574f-407d-b7f6-db903fd5aeca
y1t, y2t, w1t, w2t, yt = Q1(xt)

# ╔═╡ bbe619d2-02fe-4390-a457-b405fe434054


# ╔═╡ 6f523be5-cd7d-4769-beb3-44472abfdaa2
begin
	Y1w = fft(y1t);
    Y2w = fft(y2t);
    W1w = fft(w1t);
...
# ╔═╡ 5087d1dd-6407-4adc-b2e4-5b2dc99d9be1
md"## Plots"

# ╔═╡ b3652d2a-8304-4073-8e5e-55bc43cb7b4f
begin
	md" #### Y1(w)"
	p1 = plot()
	plot!(p1,w, real(Y1w), label = "Y1(jw)", title="Y1(jw)", ylim = [-100,100])
end

# ╔═╡ e1490cef-70f7-4ebf-a1d2-7355e0342bf2
begin
	md" #### Y2(jw)"
	p2 = plot()
	plot!(p2, w, real(Y2w), label = "Y2(jw)", title="Y2(jw)", ylim = [-100,100])
end
...
begin
	md" #### W1(jw)"
	p3 = plot()
	plot!(p3, w, real(W1w), label = "W1(jw)", title="W1(jw)", ylim = [-100,100])
end
...
begin
	md" #### W2(jw)"
	p4 = plot()
	plot!(p4, w, real(W2w), label = "W2(jw)", title="W2(jw)", ylim = [-100,100])
end
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "1b0e7e228bf1f5ea7cc1e97a04a095bea621f91a"
